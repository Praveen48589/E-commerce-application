...
const OrderDetails = ({ order }: OrderDetailsProps) => {
	...
	return (
		<motion.div
			...
			key={order._id}
			className="order-details mt-10"
		>
			<h1 className="font-medium text-lg md:text-2xl">Order #{order._id}</h1>
			...
			<div className="bg-accent my-4 rounded-lg p-4">
				<div className="flex w-full justify-between md:items-center gap-4 flex-wrap">
					<div className="flex gap-5 items-center">
						<div
							className={`${getStatusColor(order.status)} rounded-lg py-1 px-2 capitalize text-sm`}
						>
							{order.status}
						</div>
					</div>
